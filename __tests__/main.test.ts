...
describe('main', () => {
  it('calls extractProps and actionProcessor', async () => {
    const extractPropsMock = jest.spyOn(props, 'extractProps').mockImplementation(() => listProps);
    const actionProcessorMock = jest.spyOn(actionProcessor, 'processAction').mockImplementation(async (): Promise<void> => {});
  ...
  it('fails if extractProps returns undefined', async () => {
    ...
    await expect(main.run()).rejects.toEqual(JSON.stringify({ message: 'Failed to parse properties!', name: 'PropsParseError' }, null, 2));
  ...
  it('fails if processor fails', async () => {
    ...
    jest.spyOn(actionProcessor, 'processAction').mockRejectedValue({ message: 'some-error', name: 'ProcessorError' });

    await expect(main.run()).rejects.toEqual(JSON.stringify({ message: 'some-error', name: 'ProcessorError' }, null, 2));
  });
})
