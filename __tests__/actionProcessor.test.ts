...
import {ActionType, PortainerActionProps, PortainerProps, RepoProps} from "../src/props";
...
const createMockAxios = (listRes: PortainerStack[]): [AxiosInstance, AxiosMockAdapter] => {
  const axiosInstance = axios.create();
  const mockAxios = new AxiosMockAdapter(axiosInstance);

  mockAxios.onGet('http://localhost:8000/api/stacks?filters=%7B%22EndpointId%22%3A1%7D').reply(200, listRes);
...
describe('actionProcessor', () => {
  it('properly handles "List" action', async () => {
    ...
    await actionProcessor.processAction(listProps, axiosInstance).catch((e) => console.log(JSON.stringify(e)));
    ...
    expect(request.url).toBe('http://localhost:8000/api/stacks?filters=%7B%22EndpointId%22%3A1%7D');
  ...
  it('properly handles "Upsert" action: Create', async () => {
    ...
    await actionProcessor.processAction(upsertProps, axiosInstance).catch((e) => console.log(JSON.stringify(e)));
    ...
    expect(requestHeaders.get('X-API-KEY')).toBe(apiKey);
    expect(requestHeaders.get('Authorization')).toBe(undefined);
    expect(request.url).toBe('http://localhost:8000/api/stacks/create/standalone/repository?endpointId=1');
  ...
  it('properly handles "Upsert" action: Update', async () => {
    ...
    expect(requestHeaders.get('X-API-KEY')).toBe(apiKey);
    expect(requestHeaders.get('Authorization')).toBe(undefined);
    expect(request.url).toBe('http://localhost:8000/api/stacks/101/git/redeploy?endpointId=1');
    expect(JSON.parse(request.data)).toMatchObject(expectedRequestBody);
  });
  ...
  it('properly handles "Delete" action: Found Stack', async () => {
    ...
    await actionProcessor.processAction(deleteProps, axiosInstance).catch((e) => console.log(JSON.stringify(e)));
    ...
    expect(requestHeaders.get('X-API-KEY')).toBe(apiKey);
    expect(requestHeaders.get('Authorization')).toBe(undefined);
    expect(request.url).toBe('http://localhost:8000/api/stacks/101?endpointId=1');
  });
})
